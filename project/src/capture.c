...
void start_capture() 
{
    ...
    // [CR] buffer of what? how about packet_buffer?
    unsigned char data_buffer[BUFFER_SIZE]; // Buffer to store packet data
    ...
    for (;;) 
    {
        ...
        int data_size = recvfrom(sock_raw, data_buffer, BUFFER_SIZE, 0, &saddr, (socklen_t*)&saddr_len);
        ...
        parse_ethernet_frame(data_buffer);
    }
